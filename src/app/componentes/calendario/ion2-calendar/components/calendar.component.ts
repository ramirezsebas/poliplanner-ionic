...
@Component({
  ...
  template: `
    <div class="title ion-padding-top ion-padding-bottom">
      <ng-template [ngIf]="_showMonthPicker" [ngIfElse]="title">
      <ion-label
      fill="clear"
      class="switch-btn"
      style="text-transform: uppercase; text-size:24px"
      ...
      <ng-template [ngIf]="_showToggleButtons">
        <ion-button type="button" fill="clear" class="back" [disabled]="!canBack()" (click)="prev()">
          <ion-icon slot="icon-only" size="large" name="caret-back-outline"></ion-icon>
        </ion-button>
        <ion-button type="button" fill="clear" class="forward" [disabled]="!canNext()" (click)="next()">
          <ion-icon slot="icon-only" size="large" name="caret-forward-outline"></ion-icon>
        </ion-button>
      </ng-template>
    </div>
