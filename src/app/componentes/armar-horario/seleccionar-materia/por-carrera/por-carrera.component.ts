...
export class PorCarreraComponent implements OnInit {
  @Input() formData: FormService;
  @Input() selectedCareerId: number;
  ...
  ngOnInit() {
    ...
    //refactorizar: intentar sacar esta condicion
    if (this.esAprobar) {
  ...
  descartarDatosPrerrequisito() {
    this.fpuna.getPrerrequisitosAll().subscribe((prerrequisitos) => {
      const idCarrera: number = this.selectedCareerId;

      let nombreAprobadas: string[] =
        this.formData.materiasAprobadas[this.selectedCareerId];
      if (!this.formData.materiasAprobadas[this.selectedCareerId])
        nombreAprobadas = [];

      // console.log('todas', todas);
      //refactorizar: sacar este codigo
      if (this.esCargarHorario) {
        this.formData.clasesTodas = undefined;
        this.getData();
      }

      const todas = this.formData.clasesTodas.filter(
        (x) => x.career_id == idCarrera
      );

      let ids_aprobadas = todas
        .filter((x) => nombreAprobadas.includes(x.name))
        .map((x) => x.id);

      const aprobadas = todas.filter((x) => ids_aprobadas.includes(x.id));
      ...
      candidatos = candidatos.filter(
        (x) => x.credits_percentage_required <= porcentajeCreditos
      );
      // console.log('despues de creditos', candidatos);

      //Filtra prerrequisitos
      candidatos = candidatos.filter((materia) => {
        let prePorMateria: any[] = prerrequisitos
          .filter((x) => x.class1_id == materia.id)
          .map((x) => x.class2_id);
      ...
      // Excluir aprobados
      candidatos = candidatos.filter((x) => !ids_aprobadas.includes(x.id));
      ...
      this.semestersClasses.map((x) =>
        x.materias.sort((x, y) => (x.sem > y.sem ? 1 : -1))
      );
      console.log("this.semestersClasses", this.semestersClasses);

      const marcados =
        this.formData.materiasSeleccionadas[this.selectedCareerId];
      if (marcados)
        marcados.forEach((toCheck) => {
          this.semestersClasses.forEach((sem) =>
  ...
  getData() {
    this.fpuna.getClasesAll().subscribe((r) => {
      ...
      this.semestersClasses = c.sort((x, y) => x.semestre - y.semestre);
      this.semestersClasses.map((x) =>
        x.materias.sort((x, y) => x.sem > y.sem)
      );
      ...
      const marcados = this.formData.materiasAprobadas[this.selectedCareerId];
      if (marcados)
        marcados.forEach((toCheck) => {
          this.semestersClasses.forEach((sem) =>
            sem.materias.forEach((materia) => {
  ...
  estaActivo(i) {
    ...
    if (aux)
      return document
        ...
        .previousElementSibling.classList.contains("active");
    else return false;
  }
}
